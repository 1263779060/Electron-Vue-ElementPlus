<script setup>
	...
	import { useUserIdentity } from '@/stores/userIdentity'
	...
	const toCompanyCenter = () => {
		const logStatus = userIdentity.useridentity === 'company';
		const logToken = !!userToken.usertoken;
	...
	const logOut = () => {
		...
		userIdentity.setUserIdentity(null)
		location.reload()
	}

	const logTypeCheck = () => {
		const token = userToken.usertoken;
		const logStatus = userIdentity.useridentity;
		if (token && logStatus) {
...
<template>
  ...
  <div style="width: 1000px;margin: 0 auto;background-color: white" class="pt-5 pb-2">
    <el-row :gutter="10">
      <el-col :span="6">
        <div>
